        ...
        if (!readyMapPieces.IsEmpty)
        {
            ...
            while (q-- > 0)
            {
                if (readyMapPieces.TryDequeue(out var mappiece))
                {
                    ...
                    if (!loadedMapData.TryGetValue(mcord, out TopographicMultiChunkMapComponent mccomp))
                    {
                        loadedMapData[mcord] = mccomp = new TopographicMultiChunkMapComponent(api as ICoreClientAPI, baseCord, this);
                    }
    ...
    public override void OnViewChangedClient(List<FastVec2i> nowVisible, List<FastVec2i> nowHidden)
    {
        ...
        lock (chunksToGenLock)
        {
            foreach (FastVec2i cord in nowVisible)
            {
                ...
                if (loadedMapData.TryGetValue(tmpMccoord, out TopographicMultiChunkMapComponent mcomp))
                {
        ...
        foreach (FastVec2i cord in nowHidden)
        {
            ...
            if (loadedMapData.TryGetValue(mcord, out TopographicMultiChunkMapComponent mc))
            {
