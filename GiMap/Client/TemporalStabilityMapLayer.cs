...
public class TemporalStabilityMapLayer : ABlockMapLayer
{
    ...
    protected override int GetColor(BlockPos pos)
    {
        ...
        if (stability < 0.6)
            return ConfigManager.ConfigInstance.TemporalStabilityMode._colorMin;
        if (stability < 0.8)
            return ConfigManager.ConfigInstance.TemporalStabilityMode._colorLow;
        if (stability < 1)
            return ConfigManager.ConfigInstance.TemporalStabilityMode._colorMid;
        if (stability < 1.2)
            return ConfigManager.ConfigInstance.TemporalStabilityMode._colorHigh;
        
        return ConfigManager.ConfigInstance.TemporalStabilityMode._colorMax;
    }
